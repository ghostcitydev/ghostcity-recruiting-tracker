...
| PocketScout NSD Roster Plan mod log always showed "0" for every category (position changes, unsigned fills, FCS pool trades/cuts, mirror rebalances, talent rescue) even when changes were genuinely applied | `applySelectedRosterChanges` in `nsdRosterBalancer.js` returned real per-category counters (`internalPositionChangesApplied`, `unsignedRecruitFillsApplied`, etc.) but never included a `summary` object in its response. The frontend's mod-log card (`app/import/page.tsx`) reads `summary.internalPositionChangeProposals` and similar field names, so `compactRosterResult` in `route.ts` always fell back to `summary: {}` on apply, showing zero across the board regardless of how much work was done. The top-line "N confirmed roster changes applied" count was also just echoing the pre-apply request's proposal-ID count rather than what was actually reprocessed. | `applySelectedRosterChanges` now builds a `summary` object using the same field names the preview path already produces, sourced from its own applied counters (added a new `mirrorRebalancesApplied` counter, since mirror rebalances previously had no dedicated tally separate from general position changes). `route.ts` now reports `appliedProposalCount` from the roster balancer's own `selectedProposalCount` when available, falling back to the request's count only if apply didn't run. | `node --check` and `tsc --noEmit` both pass with no new errors. Recommend a local NSD apply run to confirm the mod-log category counts are now nonzero and the applied total matches `selectedProposalCount`. |
