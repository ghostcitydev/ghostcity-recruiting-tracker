...
// V0.2.1 upstream fix (io/saveFile.js) normalizes EA's renamed "MW" save
// field back to "MWC" when reading conferences, so confArray always shows
// conf.Name === "MWC" for the Mountain West going forward. But pipeline
// history recorded BEFORE that fix was applied is still keyed under the
// raw "MW"/"MWTenures" names. Without this alias fallback,
// hist[dynastyCode]["MWCTenures"]/["MWC"] is undefined for any dynasty
// with pre-fix history, and indexing [String(season)] off that undefined
// throws "Cannot read properties of undefined (reading '<season>')".
const CONF_HISTORY_KEY_ALIASES = { MWC: 'MW', MW: 'MWC' };

function historyConfKeyCandidates(confBaseName) {
  const alias = CONF_HISTORY_KEY_ALIASES[confBaseName];
  return alias ? [confBaseName, alias] : [confBaseName];
}

// Looks up a conference-keyed history bucket (conf.tenures via the
// "...Tenures" suffix, or conf.applicationStatus via no suffix), trying
// the current (post-fix) conference name first and falling back to the
// pre-fix alias if that's where the season's data actually lives.
function readConfHistory(dynasty, confBaseName, suffix, season) {
  for (const candidate of historyConfKeyCandidates(confBaseName)) {
    const bucket = dynasty && dynasty[candidate + suffix];
    if (bucket && Object.prototype.hasOwnProperty.call(bucket, String(season))) {
      return bucket[String(season)];
    }
  }
  return undefined;
}
...
function pullHistory(teamsByIndex,confArray,season,hist,dynastyCode,settings,curr){
    const dynasty = hist[dynastyCode];
    for(const team of teamsByIndex){
        team.prestigeHistory.push(team.currentPrestige);
        ...
        const teamHistory = dynasty && dynasty[team.displayName] && dynasty[team.displayName][String(season)];
        for(let i =1; i<settings.prestigeAvgLength; i++){
            // Team has no recorded entry for this prior season (e.g. it
            // just joined the dynasty via transfer/promotion) -- fall back
            // to its current prestige instead of crashing.
            team.prestigeHistory.push(teamHistory ? teamHistory[i-1] : team.currentPrestige);
        };
    };
    for(const conf of confArray){

        conf.tenures = readConfHistory(dynasty, conf.Name, "Tenures", season);
        if (!conf.tenures) {
            // No recorded tenures under either the current or pre-V0.2.1
            // alias name (e.g. a genuinely new conference) -- fall back to
            // zeroed tenures instead of crashing.
            conf.tenures = conf.memberRecords.map(() => 0);
        }
        for(let j=0;j<conf.memberRecords.length;j++){
            const t = String(conf.memberRecords[j].DisplayName);
            const c = conf.Name;
            // Team has no recorded conference for this prior season (new
            // team) -- treat as "no match" (c2 = null) rather than crash,
            // which correctly falls into the tenures[j] = 0 branch below.
            const teamConfHistory = dynasty && dynasty[t+"Conf"];
            const c2 = teamConfHistory ? teamConfHistory[String(season)] : null;
    ...
    for(const conf of confArray){
         conf.applicationStatus = readConfHistory(dynasty, conf.Name, "", season);
         if (!conf.applicationStatus) {
             // Same fallback as tenures above: no recorded application
             // status under either name -- treat every current member as
             // fully settled (100) and everyone else as uninterested (0),
             // matching setBaseline's shape for a fresh conference.
             conf.applicationStatus = teamsByIndex.map((team) => [
                 team.displayName,
                 team.confName === conf.Name ? 100 : 0,
             ]);
         }
    };
