...
function applySliders(settings) {
  ...
  next.hawaiiBonus = next.dhawaiiBonus * next.shawaiiBonus / 100;
  next.confDesiredSize = { ...next.confDesiredSize };
  for (const conference of ['ACC', 'Big Ten', 'SEC', 'Big 12']) next.confDesiredSize[conference] = next.P4confsize;
  next.confDesiredSize['Pac-12'] = next.PAC12confsize;
  for (const conference of ['Sun Belt', 'American', 'MAC', 'MWC', 'CUSA']) next.confDesiredSize[conference] = next.G5confsize;
  return next;
}
