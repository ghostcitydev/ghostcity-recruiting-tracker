...
function portraitNames(archive, skin, body) { const prefix = `\\assets\\Player_Portraits\\${skin}Generic\\${body}\\nilpp_Generic_`; return ASAR.listPackage(archive).filter((item) => item.startsWith(prefix) && /\.webp$/i.test(item)).map((item) => path.basename(item).replace(/^nilpp_/i, '').replace(/\.webp$/i, '')); }
async function run({ savePath, settings: payload, log = () => {} }) {
  ...
  if (!fs.existsSync(archive)) throw new Error('Bundled Fang resources are missing.');
  const names = enabled(settings, 'nameWeights') ? JSON.parse(ASAR.extractFile(archive, 'weightedNamePools.json').toString()) : null;
