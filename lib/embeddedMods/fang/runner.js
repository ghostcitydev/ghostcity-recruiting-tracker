...
const DATA_ROOT = [
  ...
  path.join(process.cwd(), 'lib', 'embeddedMods', 'fang', 'data'),
  path.join(process.cwd(), 'embeddedMods', 'fang', 'data')
].find((candidate) => fs.existsSync(candidate)) || path.join(__dirname, 'data');
...
async function run({ savePath, settings: payload, log = () => {} }) {
  ...
  if (!fs.existsSync(NAME_POOLS_PATH) || !fs.existsSync(PORTRAIT_PATHS_PATH)) throw new Error('Bundled Fang resources are missing.');
