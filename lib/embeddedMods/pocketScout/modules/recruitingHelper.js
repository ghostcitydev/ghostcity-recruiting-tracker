/* PocketScout NSD suppress intermediate placement CSV and expose final-report seed v11 */
/* PocketScout NSD consolidated Roster Balancer assignment preview v1 */
  ...
  async run({
    ...
    if (mode === 'hardCommitSelectedProspectBoard') {
      return hardCommitSelectedRecruitingHelperBoard({
        ...
        outputPath: outputPath || inputPath,
        selectedTeamId: options.selectedTeamId,
        clearRecruitingDealbreakers: options.clearRecruitingDealbreakers === true,
        session
      });
    }
    ...
    /* PocketScout NSD placement diagnostics exporter v1 */
    if (
      mode ===
        'exportNsdPlacementDiagnostics'
    ) {
      return assignZeroOfferTransfers({
        inputPath,
        outputPath:
    ...
    if (
      ...
      return assignZeroOfferTransfers({
        ...
        reportDirectory:
          options.reportDirectory,
        suppressPlacementReport:
          options.suppressPlacementReport === true,
        placementSettings:
          options.placementSettings ?? {},
        bypassWeekRequirement:
          options.bypassWeekRequirement === true,
...
async function synchronizeCommittedPlayerDestination({
  ...
  /* PocketScout dynamic NSD committed-count reference resolver v67 */
  /*
   * Resolve the committed-player array from the destination Team's actual
   * CommittedPlayers binary reference. Runtime table IDs can change between
   * dynasty layouts/title updates, so never assume a fixed Player[] table ID.
   */
  const preservedCommittedTableId =
    Number.parseInt(
  ...
  if (
    !Number.isInteger(
      preservedCommittedTableId
    )
  ) {
    throw new Error(
      `Could not decode Team ${selectedTeamId} CommittedPlayers table ID.`
    );
  }
  ...
  if (
    ...
    if (
      ...
      throw new Error(
        `Team ${selectedTeamId} CommittedPlayers references table ID ${preservedCommittedTableId}, but structural resolution returned table ID ${committedPlayersTableInfo.runtimeTableId}.`
      );
    }
...
function psChooseSmartTransferDestination({
  ...
  function psDiagnosticPass(passDefinition) {
    if (!diagnosticTrace) {
      return null;
    }

    let passTrace =
      diagnosticTrace.passes.find(
        item =>
          item.pass ===
            passDefinition.pass
      );
  ...
  function psDiagnosticReject(
    ...
    passTrace.teamsChecked++;
    passTrace.rejections[code] =
      (passTrace.rejections[code] ?? 0) + 1;

    const samples =
      passTrace.samples[code] ?? [];

    if (samples.length < 8) {
      samples.push({
        teamId:
          rankedTeam?.teamId ?? null,
        teamName:
          rankedTeam?.teamName ?? '',
        ...details
      });
  ...
  function psDiagnosticAccept(
    ...
    if (passTrace) {
      passTrace.teamsChecked++;
      passTrace.acceptedCandidates++;
    }

    if (diagnosticTrace) {
      diagnosticTrace.selected = {
        pass:
          passDefinition.pass,
        passLabel:
          passDefinition.label,
        ...
        projectedRosterCount:
          destination.projectedRosterCount,
        committedCount:
          destination.committedCount,
        samePositionCount:
          destination.samePositionCount,
        depthRank:
          destination.depthRank,
        hardMaximum:
          destination.hardMaximum,
        assignedAtPosition:
          destination.assignedAtPosition,
        activePositionAdditionCap:
          destination.activePositionAdditionCap
      };
    }
  }
  ...
  for (
    ...
    if (
      ...
      if (diagnosticTrace) {
        ...
        passTrace.rejections.PLAYER_BELOW_PASS_MINIMUM =
          1;
        passTrace.samples.PLAYER_BELOW_PASS_MINIMUM = [
          {
            overall,
            minimumOverall:
              passDefinition.minimumOverall
    ...
    if (
      ...
      if (diagnosticTrace) {
        ...
        passTrace.rejections.PLAYER_NOT_DEVELOPMENTAL_CLASS =
          1;
        passTrace.samples.PLAYER_NOT_DEVELOPMENTAL_CLASS = [
          { recruitClass }
        ];
      }
    ...
    for (
      ...
      if (
        ...
        psDiagnosticReject(
          passDefinition,
          rankedTeam,
          committedCount >= signingLimit
            ? 'SIGNING_LIMIT'
            : 'PASS_COMMITTED_LIMIT',
          {
            committedCount,
            maximumCommittedCount:
              passDefinition.maximumCommittedCount,
            signingLimit
          }
        );
        continue;
      ...
      if (
        ...
        psDiagnosticReject(
          passDefinition,
          rankedTeam,
          projectedRosterCount >= finalRosterLimit
            ? 'FINAL_ROSTER_LIMIT'
            : 'PASS_ROSTER_LIMIT',
          {
            projectedRosterCount,
            maximumRosterCount:
              Number.isFinite(
                passDefinition.maximumRosterCount
              )
                ? passDefinition.maximumRosterCount
                : 'Infinity',
            finalRosterLimit
      ...
      if (
        ...
        psDiagnosticReject(
          passDefinition,
          rankedTeam,
          depth.samePositionCount >= hardMaximum
            ? 'POSITION_HARD_MAXIMUM'
            : 'PROJECTED_DEPTH_LIMIT',
          {
            samePositionCount:
              depth.samePositionCount,
            hardMaximum,
...
async function assignZeroOfferTransfers({
  ...
  suppressPlacementReport = false,
  placementSettings = {},
  bypassWeekRequirement = false,
  diagnosticsOnly = false,
  diagnosticPhase = 'pre-run',
  previewOnly = false,
  session = null
}) {
  ...
  /* PocketScout NSD post-run diagnostics backend v7 */
  const normalizedDiagnosticPhase =
    String(
      diagnosticPhase ??
      'pre-run'
    ) === 'post-run'
...
/* PocketScout NSD distribution order include-user repair v3 */
  const includeUserControlledTeams =
    placementSettings
   ...
   * NIL behavior. PocketScout's historical NSD compatibility path writes
   * RecruitingDealbreaker = Invalid; retain it only as an explicit opt-in.
   */
  const clearRecruitingDealbreakers =
    placementSettings
      ?.clearRecruitingDealbreakers ===
        true;

  /* PocketScout NSD distribution order backend v2 */
  /* PocketScout NSD Prefer Top School default two-option backend v6 */
  const requestedDistributionOrder =
    String(
      placementSettings
        ?.distributionOrder ??
      'prefer-top-school'
  ...
  const distributionOrder =
    [
/* PocketScout NSD Prefer Top School distribution backend v4 */
      'prefer-top-school',
      'balanced'
    ].includes(
      requestedDistributionOrder
    )
      ? requestedDistributionOrder
      : 'prefer-top-school';

  const userControlledTeamIds =
    await resolveRecruitingHelperUserTeamIds({
      franchise,
      session,
      teamTableInfo:
        topSchoolsContext
  ...
  /*
   * PocketScout zero offer transfer committed limit v67
   *
   * Snapshot each school's current Team.CommittedPlayers usage by following
   * the Team row's authoritative binary reference. The structurally resolved
   * Player[35] table is validated once, then every Team reference must point
   * to that table's current runtime ID. No fixed table ID/index is assumed.
   */
  const committedPlayersTableInfo =
    await loadCommittedPlayersArrayTable({
      franchise,
  ...
  const committedPlayersRuntimeTableId =
    Number(
      committedPlayersTableInfo
        .runtimeTableId ??
      getRuntimeTableId(
        committedPlayersTable
      )
    );

  if (
    !Number.isInteger(
      committedPlayersRuntimeTableId
    )
  ) {
    throw new Error(
      'Could not resolve the runtime table ID for the committed-player Player[35] array.'
    );
  }
  ...
  /* PocketScout NSD preserve existing commitments dealbreaker cleanup v12 */
  const existingCommittedPlayerReferencesSeen =
    new Set();

  let existingCommittedPlayersScanned = 0;
  let existingCommittedDealbreakersCleared = 0;
  let existingCommittedPlayerReferencesUnresolved = 0;
  ...
  for (
    ...
    if (
      ...
      throw new Error(
        `Team ${teamId} CommittedPlayers references runtime table ID ${tableId}, but the structurally validated Player[35] table is runtime table ID ${committedPlayersRuntimeTableId}.`
      );
    }
    ...
    if (!committedRecord) {
      throw new Error(
        `Could not load Team ${teamId} committed-player row ${row} from runtime table ID ${committedPlayersRuntimeTableId}.`
      );
    }

    const committedPlayerFields =
      Object.keys(
        committedRecord.fields ??
        committedRecord
      )
    ...
    if (
      committedPlayerFields.length !== 35 ||
      committedPlayerFields.some(
        (fieldName, index) =>
          fieldName !== `Player${index}`
      )
    ) {
      throw new Error(
        `Team ${teamId} committed-player row ${row} does not have the expected Player0-Player34 structure.`
      );
    }

    const uniqueCommittedPlayers =
      new Set();

    for (
      const fieldName
      of committedPlayerFields
    ) {
      const playerReference =
        String(
          committedRecord[
    ...
    if (
      ...
      for (
        const playerReference
        of uniqueCommittedPlayers
      ) {
        if (
          existingCommittedPlayerReferencesSeen.has(
            playerReference
          )
        ) {
          continue;
        ...
        existingCommittedPlayerReferencesSeen.add(
          playerReference
        );

        const playerReferenceInfo =
          decodeBinaryReference(
            playerReference,
            tableIdMap
          );
        ...
        if (
          ...
          existingCommittedPlayerReferencesUnresolved++;
          continue;
        }

        existingCommittedPlayersScanned++;
        ...
        if (
          ...
          existingCommittedDealbreakersCleared++;
        }
      }
    }
  }
  ...
  for (
    ...
    if (
      ...
      /* PocketScout NSD all unsigned recruits candidate pool v47 */
      [
        'hardcommitted',
        'signed',
        'invalid'
    ...
    /* PocketScout NSD all unsigned recruits regardless of offers v48 */
  ...
  if (
    ...
    rankedTeamsByPreviousFinish.sort(
      (left, right) =>
        right.teamPrestige -
          left.teamPrestige ||
        balancedTeamComparator(
          left,
          right
        )
    );
    ...
    for (
      let index =
        rankedTeamsByPreviousFinish.length - 1;
      index > 0;
      index--
    ) {
      const swapIndex =
      ...
      [
        rankedTeamsByPreviousFinish[index],
        rankedTeamsByPreviousFinish[swapIndex]
      ] = [
        rankedTeamsByPreviousFinish[swapIndex],
        rankedTeamsByPreviousFinish[index]
      ];
  ...
  if (diagnosticsOnly) {
    ...
    const rankedTeamIds =
      new Set(
        rankedTeamsByPreviousFinish.map(
          team => team.teamId
        )
      );
    ...
    const teamRows =
      (
        ...
        .map(teamRecord => {
          ...
          let chooserExclusion =
            '';

          if (!teamOption) {
            chooserExclusion =
              'TEAM_OPTION_NOT_RESOLVED';
          } else if (
            !includeUserControlledTeams &&
            userControlledTeamIds.has(
              teamId
            ...
            chooserExclusion =
              'COMMITTED_COUNT_NOT_RESOLVED';
          } else if (
            committedCount >= 35
          ) {
            chooserExclusion =
              'PRE_RANKING_35_COMMITTED_FILTER';
          } else if (
            !Array.isArray(roster)
          ) {
            chooserExclusion =
              'ROSTER_CONTEXT_NOT_RESOLVED';
          }
          ...
          return {
            ...
            projectedRosterCount:
              Array.isArray(roster)
                ? roster.length
                : null,
            availableSigningSlots:
              Number.isInteger(
                committedCount
              )
                ? Math.max(
                    0,
                    Number(placementSettings.signingLimit ?? 35) -
                      committedCount
                  )
                : null,
            availableProjectedRosterSlots:
              Array.isArray(roster)
                ? Math.max(
                    0,
                    Number(placementSettings.finalRosterLimit ?? 95) -
                      roster.length
                  )
                : null,
            positionCounts:
    ...
    const teamsWithSigningCapacity =
      teamRows.filter(
        team =>
          Number.isInteger(
            team.committedCount
          ...
          team.committedCount <
            selectedSigningLimit
      ).length;

    const teamsWithProjectedRosterCapacity =
      teamRows.filter(
        team =>
          Number.isInteger(
            team.projectedRosterCount
          ) &&
          team.projectedRosterCount <
            selectedRosterLimit
      ).length;

    const teamsWithBothCapacity =
      teamRows.filter(
        team =>
          Number.isInteger(
            team.committedCount
    ...
    for (
      ...
      playerRows.push({
        recruitRow:
          candidate.recruitRow,
        playerRow:
          candidate.playerReference.row,
        player:
          [
            toText(
              candidate.playerRecord.FirstName
            ),
            toText(
              candidate.playerRecord.LastName
            )
          ].filter(Boolean).join(' '),
        class:
          toText(
            candidate.recruitRecord.Class
          ),
        position:
          toText(
        ...
        chooserDestination:
          result.destination
            ? {
                teamId:
                  result.destination.teamId,
                teamName:
                  result.destination.teamName,
                pass:
                  result.destination.pass,
                passLabel:
                  result.destination.passLabel,
                depthRank:
                  result.destination.depthRank,
                committedCount:
                  result.destination.committedCount,
                projectedRosterCount:
                  result.destination.projectedRosterCount
              }
            : null,
        chooserReason:
          result.reason,
    ...
    const report = {
      generatedAt:
        new Date().toISOString(),
      diagnosticType:
        normalizedDiagnosticPhase ===
          'post-run'
          ? 'NSD Assign Unsigned Players Post-Run Landscape'
          : 'NSD Assign Unsigned Players Pre-Run Smart Placement',
      diagnosticPhase:
        normalizedDiagnosticPhase,
      readOnly:
        true,
      ...
      resolvedTables: {
        ...
        team: {
          tableIndex:
            topSchoolsContext.teamTableInfo.tableIndex,
          runtimeTableId:
            getRuntimeTableId(
              topSchoolsContext.teamTableInfo.table
            )
        },
        committedPlayers: {
          tableIndex:
      ...
      counts: {
        eligibleUnsignedPlayers:
          transferCandidates.length,
        rankedTeamsAvailableToChooser:
          rankedTeamsByPreviousFinish.length,
        teamsWithSigningCapacity,
        teamsWithProjectedRosterCapacity,
        teamsWithBothCapacity,
        userControlledTeamsFound:
          userControlledTeamIds.size,
        signedClassPlayersIncluded,
        signedClassAlreadyOnRoster,
        signedClassSkippedInvalid
      },
      notes: [
        'This export does not modify or save the dynasty file.',
        normalizedDiagnosticPhase === 'post-run'
          ? 'Post-run mode reads the dynasty after assignment and shows remaining unsigned players plus each team\'s current signing, projected-roster, position, and Smart Placement capacity.'
          : 'Pre-run mode shows the landscape before assignment.',
        'Team projectedRosterCount includes the roster context used by Smart Placement after signed/hard-committed incoming players are added.',
        'Per-player trace records the exact pre-board rejection gates used by the four Smart Placement passes.',
        'Recruiting-board replacement is intentionally not executed in diagnostic mode because ensureSelectedTeamRecruitTarget can allocate or replace RecruitTarget rows.'
      ],
      teams:
        teamRows,
      players:
    ...
    const diagnosticsPath =
      path.join(
        ...
        normalizedDiagnosticPhase ===
          'post-run'
          ? `NSD_Assign_Unsigned_Post_Run_Diagnostics_${timestamp}.json`
          : `NSD_Assign_Unsigned_Pre_Run_Diagnostics_${timestamp}.json`
      );
  ...
  function psPreferTopSchoolDestination({
    recruitRow,
    playerRecord,
    originalTopSchool
  }) {
    ...
    const rankedTeam =
      rankedTeamsByPreviousFinish.find(
        team =>
          team.teamId ===
            teamId
      );
  ...
  for (
    ...
    if (
      ...
      const preferredTopSchool =
        psPreferTopSchoolDestination({
          recruitRow,
          playerRecord,
          originalTopSchool
        });

      if (preferredTopSchool) {
        destination =
          preferredTopSchool;

        reason =
          `Prefer Top School: ${preferredTopSchool.teamName} is the player's current #1 recruiting school and has room under the selected signing, projected roster, and position hard-maximum limits.`;

        projectedDepthRank =
          preferredTopSchool.depthRank;

        playersAhead =
          preferredTopSchool.playersAhead;

        teamStrength =
          Math.round(
            preferredTopSchool.teamStrength *
              10
          ) / 10;
      }
  ...
  if (
    !previewOnly &&
    existingCommittedDealbreakersCleared > 0 &&
    !(
      transfersAssigned > 0 &&
      resolvedRosterMoves.length
    )
    ...
    backupResult =
      backupBeforeSave(
        resolvedInput,
        'before-nsd-existing-commitment-dealbreaker-cleanup'
      );

    if (backupResult.backupError) {
      rosterSyncWarnings.push(
        'Could not create a backup before saving existing committed-player dealbreaker cleanup: ' +
    ...
    resolvedRosterMoves.map(
      move => ({
        ...
        teamName:
          ...
          `Team ${move.toTeamIndex}`,

        assignmentPass:
          move.assignmentPass ?? '',

        assignmentDecision:
          move.assignmentDecision ?? ''
      })
    );

  const previewAssignmentsByTeam =
    new Map();
  ...
  for (
    ...
    teamPreview.assignments.push(
      assignment
    );

    teamPreview.positionCounts[
      assignment.position
    ] =
      (
        teamPreview.positionCounts[
  ...
  const previewTeams =
    [...previewAssignmentsByTeam.values()]
      .map(
        teamPreview => ({
          ...teamPreview,
          assignmentCount:
            teamPreview.assignments.length,
          projectedCommittedCount:
            committedCountByTeam.get(
              teamPreview.teamIndex
            ) ?? null,
          projectedRosterCount:
            rosterByTeam.get(
              teamPreview.teamIndex
...
/* PocketScout Prospect Board commit visibility and user team default */
/* PocketScout NSD Assign Unsigned exclude user-controlled teams v1 */
async function resolveRecruitingHelperUserTeamIds({
  franchise,
  session,
  teamTableInfo
}) {
  ...
  const userCoachReferences =
    new Set();

  for (
    let coachRow = 0;
    coachRow <
      (coachTable.records ?? []).length;
    coachRow++
  ) {
    const coachRecord =
      coachTable.records?.[
  ...
  for (
    ...
    const headCoach =
      String(
        teamRecord.HeadCoach ??
        ''
      );

    const userCharacter =
      String(
        teamRecord.UserCharacter ??
        ''
      );

    if (
      !userCoachReferences.has(
        headCoach
      ) &&
      !userCoachReferences.has(
        userCharacter
...
export async function runRecruitMassEdit({
  ...
  if (
    ...
    const playerRecordsForAction =
      (
        action === 'dealbreaker' &&
        onlyEditRecruits === true
      )
        ? [
            ...new Set(
              linkedRows
                .map(row => row.playerRecord)
                .filter(Boolean)
            )
          ]
        : fullPlayerRecords;
