...
export async function scanDynastyFile({
  ...
  // The Signing Day assignment only needs Team and Recruit data. Some valid
  // dynasty saves do not expose a user-controlled Coach record with the full
  // Recruiting Helper signature, so do not reject the save on that basis.
  let userRecruitingCoachTable = null;
  try {
    userRecruitingCoachTable =
    ...
    onProgress({
      stage: 'coach-table-warning',
      message:
        `No compatible user Coach table found; continuing without it: ` +
        `${error?.message ?? error}`
    });
  }
